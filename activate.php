...
$rs_check = mysql_query("select user_id from users where verificationlink='$user' and verificationcode='$activ'") or die (mysql_error()); 
...
$rs_activ = mysql_query("update users set verified_date='".date("Y-m-d H:i:s")."', `status`='1' WHERE verificationlink='$user' AND verificationcode = '$activ' ") or die(mysql_error());
...
$rs_check = mysql_query("select user_id from users where user_email='$user_email' and verificationcode='$activ'") or die (mysql_error()); 
$num = mysql_num_rows($rs_check);
  // Match row found with more than 1 results  - the user is authenticated. 
    if ( $num <= 0 ) { 
	$err[] = "Sorry no such account exists or activation code invalid.";
...
if(empty($err)) {
	$rs_activ = mysql_query("update users set verified_date='".date("Y-m-d H:i:s")."', `status`='1' WHERE 
						 user_email='$user_email' AND verificationcode = '$activ' ") or die(mysql_error());

...
if($get['cmd'] == 'approve')
{
mysqli_query($link,"update users set approved='1' where id='$get[id]'") or die(mysqli_error());
$rs_email = mysqli_query($link,"select user_email from users where id='$get[id]'") or die(mysqli_error());
...
$rs_usr_duplicate = mysqli_query($link,"select count(*) as total from `users` where `user_name`='$get[user_name]' and `id` != '$get[id]'") or die(mysqli_error());
...
$rs_eml_duplicate = mysqli_query($link,"select count(*) as total from `users` where `user_email`='$get[user_email]' and `id` != '$get[id]'") or die(mysqli_error());
...
where `id`='$get[id]'") or die(mysqli_error());
//header("Location: $ret"); 

if(!empty($get['pass'])) {
$hash = PwdHash($get['pass']);
mysqli_query($link,"update users set `pwd` = '$hash' where `id`='$get[id]'") or die(mysqli_error());
}
